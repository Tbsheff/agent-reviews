...
If zero comments are returned, print "No unanswered comments found" and start watch mode unless the user explicitly opted out.
...
### Step 3: Ask One Comment at a Time

After triage, keep the full batch in mind for context, but do not ask the user to approve the whole batch at once. Iterate through comments one at a time in stable order from the expanded output. For each comment, present only that comment's evidence, tradeoffs, recommendation, and closeout options, then call the structured question tool before moving to the next comment.

Use this structure for each comment:

```text
## PR Review Triage - Comment {current} of {total}
...
Before Phase 2 execution, read back every Approval Record and follow them exactly. If any comment lacks an Approval Record, return to Phase 1 Step 3 and ask about that comment with the mandatory user-question tool checkpoint.
